...
def identify_product(title):
    ...
    new_words = []
    exclude = ["TWS", "A1", "A2", "B3", "B4", "C5", "C6", "E8", "F9", "TV"]
    cap = False
    for x in words:
        ...
        if tag[1] in ("VBZ", "VBP", "VBG", "NNS", "VB") and tag[0].lower() not in ("does", "charges"):
            if attr == "display_size":
                ...
                try:
                    for b in range(a + 1, a + 20):
                        tag2 = tags[b]
                        if tag2[1] == "NN" and tags[b - 1][0] == "graphics" and tags[b - 2][1] == "CD" and tags[b - 3][
                            1] == "NNP":
                ...
                try:
                    for b in range(a + 1, a + 20):
                        tag2 = tags[b]
                        if tag2[1] in ("NNP", "NN", "JJ") and tag2[0] == attr and tags[a - 1][0].lower() in (
                        "no", "not"):
                            return False
                        elif tag2[1] in ("NNP", "NN", "JJ") and tag2[0] == attr and tags[a - 1][0].lower() not in (
                        "no", "not"):
                            return True
                except IndexError:
                    continue
