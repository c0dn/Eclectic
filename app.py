...
import report
from file import Photo, AttachedFile, upload
from users import Staff, Customer, Address
from session import PassToken, Session
...
def get_file(id):
    ...
    for x in file_list:
        if x.get_id() == id and isinstance(x, Photo):
        ...
        elif x.get_id() == id and is_authenticated(request) and isinstance(x, AttachedFile):
            ...
            if x.get_uploaded_by().get_id() == user.get_id() or isinstance(user, Staff):
                ...
                if count == len(f_list):
                    return abort(403)
        elif x.get_id() == id and not is_authenticated(request) and isinstance(x, AttachedFile):
            return abort(403)
        else:
            counter += 1
...
def login_validate():
    ...
    for user in user_list:
        if isinstance(user, Customer) and user.email == dat["username"].lower() and user.Check_password(
              dat["password"]):
            return jsonify({"success": "true", "user": user.get_name()})
        elif isinstance(user, Staff) and user.get_staff_id() == dat["username"].lower() and user.Check_password(
              dat["password"]):
            return jsonify({"success": "true", "user": user.get_name()})
        else:
            counter += 1
    ...
    if is_authenticated(request) and is_staff(request):
        ...
        for user in user_list:
            if isinstance(user, Customer):
                results.append(user)
        return render_template("pages/staff_pages/user_management.html", users=results, user=get_user(request),
                               staff=is_staff(request))
    elif is_authenticated(request) and not is_staff(request):
        return abort(403)
    else:
    ...
    if is_authenticated(request):
        ...
        for user in user_list:
            if user.get_id() == c_user.get_id():
                if dat["data"] in e_list:
                    return jsonify(
                        {"success": "false", "message": "Your email address cannot match with a existing user"})
                elif email_regex.fullmatch(dat["data"]):
                    user.email = dat["data"]
                    dat_loader.write_data("Users", user_list, False)
...
def customer_account_manage_pass():
    if is_authenticated(request):
        ...
        if request.method == "GET":
            return render_template("pages/account_settings_password.html", staff=is_staff(request),
                                   user=get_user(request), form=form)
        elif request.method == "POST" and form.validate_on_submit():
            c_user = get_user(request)
            user_list = dat_loader.load_data("Users")["data"]
            for user in user_list:
...
def customer_account_manage_address():
    if is_authenticated(request) and not is_staff(request):
        ...
        if request.method == "GET":
            ...
            return render_template("pages/customer_pages/account_settings_address.html", form=form,
                                   staff=is_staff(request), user=user)
...
def new_ticket():
    if is_authenticated(request) and not is_staff(request):
        form = NewTicketForm()
        if request.method == "GET":
            return render_template("pages/customer_pages/ticket_create.html", staff=is_staff(request),
                                   user=get_user(request), form=form)
        elif request.method == "POST" and form.validate_on_submit():
            user = get_user(request)
    ...
    if is_authenticated(request):
        ...
        if request.method == "GET":
            ...
            for ticket in ticket_list:
                if ticket.get_id() == id:
                    if ticket.get_staff_usr_id() == user.get_id() or ticket.created_by.get_id() == user.get_id():
                        return render_template("pages/ticket_detail.html", ticket=ticket, user=user,
                                               staff=is_staff(request), form=form)
                    else:
                        return abort(403)
                else:
                    count += 1
    ...
    if is_authenticated(request) and not is_staff(request):
        ...
        for cart in cart_list:
            if cart.get_user() == user.get_id():
                return render_template("pages/customer_pages/view_cart.html", cart_total=cart.get_total(),
                                       user=get_user(request),
                                       cart_items=cart.get_items(),
